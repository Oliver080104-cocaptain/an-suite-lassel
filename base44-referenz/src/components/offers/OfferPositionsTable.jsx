...
import { Trash2, Plus, GripVertical, Check, ChevronsUpDown, FileText } from "lucide-react";
...
export default function OfferPositionsTable({ positions, onChange, readOnly = false }) {
  ...
  const handleChange = (index, field, value) => {
    ...
    // Konvertiere numerische Felder zu Zahlen
    let parsedValue = value;
    if (['menge', 'einzelpreisNetto', 'rabattProzent', 'ustSatz'].includes(field)) {
          ...
          <TableBody>
            {positions.map((pos, index) => (
              <TableRow key={index} className="group">
                    ...
                    <div className="flex flex-col gap-1 w-full">
                      ...
                      {pos.produktName && (
                        <input
                          ...
                          placeholder="Titel anpassen (z.B. Taubenabwehr Innenhof)..."
                        />
                      )}
                    </div>
                  )}
                ...
                <TableCell>
                  {readOnly ? (
                    <CurrencyDisplay value={pos.einzelpreisNetto} className="text-right block text-xs sm:text-base" />
                  ) : (
                    <Input
                      type="number"
                      step="0.01"
                ...
                <TableCell className="text-right">
                  <CurrencyDisplay value={pos.gesamtNetto} className="font-medium text-xs sm:text-base" />
                </TableCell>
                {!readOnly && (
