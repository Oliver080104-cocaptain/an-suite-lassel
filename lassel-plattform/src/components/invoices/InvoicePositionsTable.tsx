...
interface Props {
  ...
  readOnly?: boolean
  showTeilfaktura?: boolean
}

export default function InvoicePositionsTable({ positions, onChange, readOnly = false, showTeilfaktura = false }: Props) {
  ...
  const handleChange = (index: number, field: keyof Position, value: any) => {
    const updated = [...positions]
    const numericFields = ['menge', 'einzelpreisNetto', 'rabattProzent', 'ustSatz', 'teilfakturaProzent', 'bereitsFakturiert']
        ...
        <Table className="min-w-full text-sm sm:text-base">
          <TableHeader>
            <TableRow className="bg-slate-50">
              ...
              {showTeilfaktura && (
                <>
                  <TableHead className="hidden sm:table-cell w-24 text-right text-xs sm:text-sm">Teilfakt. %</TableHead>
                  <TableHead className="hidden sm:table-cell w-32 text-right text-xs sm:text-sm">Bereits fakt.</TableHead>
                </>
              )}
                    ...
                    <div className="flex flex-col gap-1 w-full">
                      ...
                      {pos.produktName && (
                        <input
                          ...
                          className="w-full text-xs text-slate-500 border-0 border-b border-dashed border-slate-300 bg-transparent focus:outline-none focus:border-emerald-400 placeholder:text-slate-300 px-1 py-0.5"
                ...
                {showTeilfaktura && (
                  <>
                    ...
                    <TableCell className="hidden sm:table-cell text-right text-slate-500">
                      <CurrencyDisplay value={pos.bereitsFakturiert as number || 0} />
                    </TableCell>
                  </>
                )}
