...
export default function InvoiceDetailPage() {
  ...
  useEffect(() => {
    if (existingInvoice && !invoiceInitialized.current) {
      setInvoice({
        ...
        kundeEmail: existingInvoice.kunde_email || '',
        emailRechnung: existingInvoice.email_rechnung || '',
  ...
  const buildRechnungData = (inv: typeof defaultInvoice, t: typeof totals) => ({
    ...
    kunde_email: inv.kundeEmail || null,
    email_rechnung: inv.emailRechnung || null,
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!isNew && (
              <Button
                ...
                title={invoice.pdfUrl ? 'PDF herunterladen' : "Zuerst 'Speichern & in Zoho ablegen' klicken"}
                onClick={() => {
                  if (invoice.pdfUrl) window.open(`/api/pdf/rechnung/${invoiceId}?download=1`, '_blank')
                }}
              >
        ...
        {!isNew && (
          <div className="rounded-xl border border-slate-200 bg-white shadow p-6">
            ...
            <div className="mt-3 text-right">
              <a
                href={`/api/pdf/rechnung/${invoiceId}`}
                target="_blank"
